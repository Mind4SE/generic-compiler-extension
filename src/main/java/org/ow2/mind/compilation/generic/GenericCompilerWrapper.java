...
package org.ow2.mind.compilation.generic;
...
public class GenericCompilerWrapper implements CompilerWrapper {
	...
	public PreprocessorCommand newPreprocessorCommand(
		...
		return new GenericPreprocessorCommand(context);
	}
	public CompilerCommand newCompilerCommand(final Map<Object, Object> context) {
		return new GenericCompilerCommand(context);
	}
	public AssemblerCommand newAssemblerCommand(final Map<Object, Object> context) {
		return new GenericAssemblerCommand(context);
	}
	public LinkerCommand newLinkerCommand(final Map<Object, Object> context) {
		return new GenericLinkerCommand(context);
	}


	protected class GenericPreprocessorCommand extends AbstractPreprocessorCommand {
		public String executable = null;
		protected GenericPreprocessorCommand(final Map<Object, Object> context) {
			super(CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0)[0], context);
			if (CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0).length == 2) {
				executable=CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0)[1];
			}
		}
		...
		public boolean exec() throws ADLException, InterruptedException {
			...
			cmd.add(this.cmd);
			if (executable != null) {
				cmd.add("--executable="+executable);
			}
			cmd.add("--preprocess");

			for (String flag : flags) {
				cmd.add("--compilation-flags="+flag);
			}
			if (dependencyOutputFile != null) {
				cmd.add("--dependency-file="+dependencyOutputFile.getPath());
			}
			for (String def : defines) { 
				cmd.add("--define="+def);
			}
			for (File incDir : includeDir ) { 
				cmd.add("--include-dir="+incDir.getAbsolutePath().trim());
			}
			for (File incFile : includeFile ) { 
				cmd.add("--include-file="+incFile.getAbsolutePath().trim());
			}
			cmd.add("--output-file="+outputFile.getPath());

			cmd.add("--input-file="+inputFile.getPath());
	...
	protected class GenericCompilerCommand extends AbstractCompilerCommand {
		public String executable = null;
		protected GenericCompilerCommand(final Map<Object, Object> context) {
			super(CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0)[0], context);
			if (CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0).length == 2) {
				executable=CompilerContextHelper.getCompilerCommand(context).split(java.io.File.pathSeparator,0)[1];
			}
	...
	protected class GenericAssemblerCommand extends AbstractAssemblerCommand {
		public String executable = null;
		protected GenericAssemblerCommand(final Map<Object, Object> context) {
			super(CompilerContextHelper.getAssemblerCommand(context).split(java.io.File.pathSeparator,0)[0], context);
			if (CompilerContextHelper.getAssemblerCommand(context).split(java.io.File.pathSeparator,0).length == 2) {
				executable=CompilerContextHelper.getAssemblerCommand(context).split(java.io.File.pathSeparator,0)[1];
			}
		}
	...
	protected class GenericLinkerCommand extends AbstractLinkerCommand {
		public String executable = null;
		protected GenericLinkerCommand(final Map<Object, Object> context) {
			super(CompilerContextHelper.getLinkerCommand(context).split(java.io.File.pathSeparator,0)[0], context);
			if (CompilerContextHelper.getLinkerCommand(context).split(java.io.File.pathSeparator,0).length == 2) {
				executable=CompilerContextHelper.getLinkerCommand(context).split(java.io.File.pathSeparator,0)[1];
			}
		}
